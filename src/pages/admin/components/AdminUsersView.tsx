...
function dangerousStatus(status: string) {
  return ["REJECTED", "SUSPENDED"].includes(status.toUpperCase());
}
...
export function AdminUsersView() {
  ...
  const openEdit = (user: UserManagementUser) => {
    ...
    setForm({
      ...
      status: user.status || defaultStatusForRole(user.role),
      password: "",
  ...
  const submitForm = async () => {
    ...
    try {
      if (modal.mode === "create") {
        ...
        await userService.updateUser(modal.user.id, buildUpdatePayload());
  ...
  const changeStatus = async (user: UserManagementUser, status: string) => {
    if (!status || status === user.status) return;
    if (dangerousStatus(status) && !window.confirm(`Change ${user.fullName} status to ${labelValue(status)}?`)) return;
    setMutating(true);
    try {
                ...
                <tr key={user.id} style={{ borderBottom: index < users.length - 1 ? `1px solid ${COLORS.border}` : "none" }}>
                  ...
                  <Td>
                    <div className="flex flex-col gap-2 items-start">
                      <StatusBadge status={normalizeBadgeValue(user.status)} />
                      <select
                        value={user.status}
                        disabled={mutating}
                        ...
                        style={{ border: `1px solid ${COLORS.border}`, background: COLORS.bg, color: COLORS.textPrimary, fontSize: 12 }}
                      >
                        <option value={user.status}>{labelValue(user.status)}</option>
                        {STATUS_OPTIONS.filter(status => status.value !== user.status).map(status => <option key={status.value} value={status.value}>{status.label}</option>)}
                      </select>
                  ...
                  <Td>{formatDate(user.joinedAt ?? user.createdAt)}</Td>
                  <Td>
...
function UserFormModal({
  ...
  const handleRoleChange = (nextRole: string) => {
    ...
    setForm(prev => ({
      ...prev,
      role: normalizedRole,
      status: defaultStatusForRole(normalizedRole),
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(30,15,5,0.28)", zIndex: 70 }}>
      <Card className="p-5" style={{ width: "min(640px, 100%)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormSelect label="Status" value={form.status} error={fieldErrors.status} onChange={value => setForm(prev => ({ ...prev, status: value }))}>
            {STATUS_OPTIONS.map(status => <option key={status.value} value={status.value}>{status.label}</option>)}
          </FormSelect>
          {role === "FPT_STUDENT" && (
            <FormInput
