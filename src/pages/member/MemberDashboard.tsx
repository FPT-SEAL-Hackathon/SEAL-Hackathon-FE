...
function mapEvent(event: EventResponse): MemberEvent {
  const participantStatus = normalizeParticipationStatus(
  ...
  return {
    id: event.eventId,
    eventName: event.eventName ?? event.name ?? event.title ?? "Untitled Event",
    description: event.description ?? "",
    startDate: event.eventStartDate,
    endDate: event.eventEndDate,
    registrationDeadline: event.registrationEnd,
    location: event.location,
    eventStatus: event.eventStatus?.eventStatusName ?? event.eventStatusName ?? event.status ?? "UNKNOWN",
    participants: "N/A",
    tracks: "N/A",
    prizePool: "N/A",
    participantId: event.eventParticipantId ?? event.participantId,
    participantStatus,
...
function mergeEventParticipation(event: MemberEvent, participation?: EventParticipantResponse): MemberEvent {
  ...
  return {
    ...event,
    participantId: participation.eventParticipantId ?? participation.participantId ?? event.participantId,
    participantStatus: statusFromParticipation(participation),
    rejectedReason: participation.rejectedReason ?? event.rejectedReason,
    appliedAt: participation.appliedAt ?? participation.registeredAt ?? event.appliedAt,
...
function statusFromParticipation(participation: EventParticipantResponse): EventCardParticipationStatus {
  return normalizeParticipationStatus(participation.status);
}
...
export function MemberDashboard({ currentPage, onNavigate }: { currentPage: string; onNavigate: (p: string) => void }) {
  ...
  const handleSubmitWork = async () => {
    if (activeTeamContext?.eventId && !isApprovedParticipationStatus(submissionParticipation?.status)) {
  ...
  const handleRegisterEvent = async (eventId: string) => {
    ...
    const targetEvent = apiEvents.find(event => event.id === eventId);
    if (!targetEvent) {
    ...
    const participantStatus = normalizeParticipationStatus(participations[eventId]?.status ?? targetEvent.participantStatus);
    ...
    try {
      ...
      setApiEvents(prev => prev.map(event => event.id === eventId ? {
        ...event,
        participantId: participation.eventParticipantId ?? participation.participantId ?? event.participantId,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="p-5 col-span-1">
          ...
          <div className="rounded-xl p-4 mb-4" style={{ background: `${COLORS.error}10`, border: `1px solid ${COLORS.error}30` }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, color: COLORS.textPrimary, margin: "8px 0" }}>{apiEvents[0]?.registrationDeadline ? "Deadline available" : "N/A"}</div>
            <div style={{ fontSize: 12, color: COLORS.textSecondary }}>Deadline: {apiEvents[0]?.registrationDeadline || "No deadline data"}</div>
          </div>
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {apiEvents.map(ev => {
          ...
          const participantStatus = normalizeParticipationStatus(participation?.status ?? ev.participantStatus);
            ...
            <Card key={ev.id} className="p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[
                  ...
                  { label: "Deadline", value: ev.registrationDeadline || "N/A", icon: <Calendar size={13} /> },
              ...
              <div className="flex gap-2">
                {isActiveParticipant ? (
                  <>
                    ...
                    <Button variant="primary" size="sm" icon={<ExternalLink size={13} />} onClick={() => setSelectedEventDetailId(ev.id)}>Enter Event</Button>
                  </>
